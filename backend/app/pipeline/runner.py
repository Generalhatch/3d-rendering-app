...
from .ingest import load_point_cloud, write_decimated_ply
from .slicing import detect_floor, extract_wall_band
from .segment import extract_wall_planes
...
def run_pipeline(
    ...
    """Full pipeline: ingest → align → rooms → fixtures → export."""
    ...
    try:
        ...
        _emit(job_id, "ingest", f"Scan loaded: {len(scan_pcd.points):,} points", 0.10)

        # Write decimated PLY for browser
        ...
        # ── 2. DXF parsing ────────────────────────────────────────────────────
        _emit(job_id, "ingest", "Parsing plan…", 0.15)
        plan_lines = extract_wall_lines(str(plan_path))
        if len(plan_lines) == 0:
            raise ValueError("No line entities found in the DXF plan file.")

        # ── 3. Alignment ──────────────────────────────────────────────────────
        _emit(job_id, "align", "Detecting floor plane…", 0.20)
        _emit(job_id, "align", "Extracting wall band…", 0.25)
        _emit(job_id, "align", "Running RANSAC plane detection…", 0.35)
        _emit(job_id, "align", "Computing principal axes…", 0.45)
        ...
        if rooms and len(plan_lines) > 0:
            # Apply transform to wall-band points for match-quality computation
            T = result.transformation
            wall_band = extract_wall_band(scan_pcd, result.floor, band_low_m, band_high_m)
            pts = np.asarray(wall_band.points)
            pts_h = np.hstack([pts, np.ones((len(pts), 1))])
            aligned_pts = (T @ pts_h.T).T[:, :3]            rooms = compute_match_quality(rooms, aligned_pts, plan_lines)
        ...
        try:
            # Apply alignment transform to the full scan
        ...
        try:
            scan_pts_2d = aligned_pts[:, :2] if len(rooms) > 0 else np.array([[]])
        ...
        _emit(
            ...
            f"{len(rooms)} rooms · {len(fixtures)} fixtures",
            1.0,
        )
